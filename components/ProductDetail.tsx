    ...
    return (
        <div className="bg-background min-h-screen pb-40">
            ...
            <div className="fixed top-0 left-0 right-0 z-50 p-4 flex justify-between items-center max-w-md mx-auto">
                <button
                    onClick={() => router.back()}
                    className="w-10 h-10 flex items-center justify-center bg-[#1c1c1c]/90 backdrop-blur-xl rounded-none shadow-lg text-[#c5a059] border border-[#c5a059]/30"
                >
            ...
            <div className="space-y-4">
                <div className="relative aspect-[3/4] w-full bg-[#1c1c1c]/5 overflow-hidden border-b border-[#c5a059]/10">
                    {imgUrl ? (
                        <Image
                            src={imgUrl}
                            alt={product.name}
                    ...
                    <div className="flex gap-3 px-6 overflow-x-auto scrollbar-hide pt-2">
                        {galleryItems.map((img, idx) => (
                            <button
                                ...
                                className={`relative min-w-[70px] h-[90px] rounded-none overflow-hidden border-2 transition-all ${activeImage === img ? "border-[#c5a059] shadow-md" : "border-[#1c1c1c]/10 bg-[#1c1c1c]/5"}`}
                            >
                                <Image
            ...
            <div className="p-6 space-y-6 flex flex-col items-start text-left w-full" dir="ltr">
                ...
                <div className="w-full text-center mt-[-10px] pb-4">
                    <h1 className="text-3xl font-black text-[#1c1c1c] leading-tight">{product.name}</h1>
                </div>

                {/* Price */}
                <div className="w-full flex justify-start">
                    <p className="text-2xl font-black text-[#1c1c1c] border-b-2 border-[#c5a059]/30 inline-block py-1">
                ...
                {availableSizes.length > 0 && (
                    <div className="space-y-4 w-full">
                        <h2 className="text-xs font-bold text-[#1c1c1c] uppercase tracking-widest border-b border-[#c5a059]/20 pb-2 w-full text-left">Taille</h2>
                                ...
                                return (
                                    <button
                                        ...
                                        className={`px-4 py-1.5 rounded-none font-bold text-[12px] transition-all border-2 ${isSelected
                                            ? "bg-[#1c1c1c] border-[#c5a059] text-[#c5a059] shadow-md shadow-black/20"
                ...
                {product.description && (
                    <div className="space-y-3 pt-2 w-full text-left">
                        <h3 className="font-bold text-[#1c1c1c] uppercase tracking-widest text-xs border-b border-[#c5a059]/20 pb-2 text-left">Détails du produit</h3>
                        <p className="text-[#1c1c1c]/80 text-[14px] leading-[1.8] whitespace-pre-wrap font-medium text-left">{product.description}</p>
                    </div>
                )}
                    ...
                    <div className="pt-8 space-y-6">
                        ...
                        <div className="flex gap-4 overflow-x-auto pb-4 -mx-1 px-1 scrollbar-hide">
                            {relatedProducts.map((item) => (
                                ...
                                >
                                    <div className="relative aspect-[3/4] rounded-none overflow-hidden bg-[#1c1c1c]/5 border border-[#c5a059]/10">
                                        {item.image ? (
                                            <Image
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-xs font-black text-[#1c1c1c]">{item.price.toLocaleString()} DA</p>
                                    </div>
                                </Link>
                            ))}
                        </div>
                ...
                <div className="fixed bottom-[calc(56px+env(safe-area-inset-bottom))] left-0 right-0 p-3 bg-[#1c1c1c] border-t border-[#c5a059]/30 w-full max-w-md mx-auto z-40" dir="ltr">
                    <div className="flex gap-3">
                        <div className="flex items-center bg-[#1c1c1c] border border-[#c5a059]/20 rounded-none h-[54px] w-1/3 min-w-[110px] shadow-sm">
                            <button
                                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                                className="w-10 h-10 rounded-none flex items-center justify-center text-white hover:bg-white/10 transition-colors"
                                aria-label="Réduire la quantité"
                            >
                                <Minus className="h-4 w-4" />
                            </button>
                            <span className="font-bold text-base w-8 text-center text-white">{quantity}</span>
                            <button
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-2 h-[54px] rounded-none font-black text-white transition-all shadow-md active:scale-95 ${availableSizes.length === 0 ? "bg-gray-200 shadow-none text-gray-500 cursor-not-allowed" : "bg-[#1c1c1c] border border-[#c5a059]/30 hover:bg-[#2a2a2a] text-[#c5a059] text-lg"
                                }`}
                        >
