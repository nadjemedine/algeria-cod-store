                    ...
                    {selectedGroup.products
                        ...
                        .map((product) => {
                            const imgUrl = product.image
                                ...
                                : undefined;
                            return (
                                <div key={product._id} className="bg-background rounded-[32px] shadow-sm border border-[#c5a059]/10 overflow-hidden flex flex-col group transition-all hover:shadow-xl hover:shadow-primary/5">
                                    <Link href={`/product/${product._id}`} className="relative aspect-[3/4] w-full overflow-hidden bg-[#c5a059]/5 block active:scale-95 transition-transform duration-300">
                                        {imgUrl ? (
                                            <Image
                                                src={imgUrl}
                                                alt={product.name}
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-[#c5a059]/5 text-gray-200">
                                                <span className="text-4xl font-black">{product.name.charAt(0)}</span>
                                            </div>
                                        )}
                                        <div className="absolute top-3 right-3 bg-background/90 backdrop-blur-md px-2 py-1 rounded-none shadow-sm border border-[#1c1c1c]/10">
                                            <p className="font-black text-[#1c1c1c] text-[10px]">{product.price.toLocaleString()} DA</p>
                                        </div>
                                    </Link>
            ...
            <div className="grid grid-cols-2 gap-8">
                {groups.map((group) => {
                    ...
                    return (
                        ...
                        >
                            <div className="relative w-full aspect-square rounded-[48px] overflow-hidden shadow-xl shadow-gray-200/50 border-4 border-background bg-[#c5a059]/5 transition-all duration-500 group-hover:scale-105 group-hover:shadow-2xl group-hover:shadow-primary/20 group-hover:border-primary/20">
                                {imgUrl ? (
                                    <Image
                                        src={imgUrl}
                                        alt={group.title}
